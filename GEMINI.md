...
This file provides guidance to the Gemini / Antigravity agent when working with code in this repository.

## Project Overview

Monorepo with two Next.js frontends and one NestJS backend for a complimentary tracker system.

## Business Context / MVP Scope

The goal of this system is to **track complimentary item distribution** (e.g., posters, keychains) to customers who purchase movie tickets. 

Key Modules:
1. **Back Office:** Manage giveaway items, cinema branches, campaigns (movie → giveaway rules), and branch-level stock.
2. **Front Office (Counter App):** Scan ticket QR codes, validate eligibility, and issue giveaways while preventing duplicate claims and auto-deducting stock. 

For full MVP requirements, features, and constraints, refer to [`docs/REQUIREMENTS.md`](docs/REQUIREMENTS.md).
